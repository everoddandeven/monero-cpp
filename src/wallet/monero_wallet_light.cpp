...
namespace monero {
  ...
  monero_subaddress monero_wallet_light::get_address_index(const std::string& address) const {
    ...
    if (m_subaddrs.m_all_subaddrs != boost::none) {
      for (const auto &kv : m_subaddrs.m_all_subaddrs.get()) {
        uint32_t account_idx = kv.first;
        for(const auto &index_range : kv.second) {
          auto subaddresses = monero_wallet_keys::get_subaddresses(account_idx, index_range.to_subaddress_indices());
          
          for (auto &subaddress : subaddresses) {
            if (_address != subaddress.m_address.get()) continue;
  ...
  void monero_wallet_light::init_subaddress(monero_subaddress& subaddress) const {
    ...
    subaddress.m_num_unspent_outputs = m_output_container.get_num_unspent(account_idx, subaddress_idx);
    subaddress.m_is_used = m_output_container.is_used(account_idx, subaddress_idx);
    subaddress.m_num_blocks_to_unlock = get_subaddress_num_blocks_to_unlock(account_idx, subaddress_idx);
  }
  
  void monero_wallet_light::init_subaddresses(std::vector<monero_subaddress>& subaddresses) const {
  ...
  std::vector<monero_subaddress> monero_wallet_light::get_subaddresses(const uint32_t account_idx, const std::vector<uint32_t>& subaddress_indices) const {
    ...
    init_subaddresses(subaddresses);
    return subaddresses;
  }
  ...
  std::vector<std::string> monero_wallet_light::relay_txs(const std::vector<std::string>& tx_metadatas) {
    ...
    for (const auto& tx_metadata : tx_metadatas) {
      ...
      m_tx_container.set_unconfirmed(tx);
  ...
  std::vector<std::string> monero_wallet_light::submit_txs(const std::string& signed_tx_hex) {
    std::cout << "monero_wallet_light::submit_txs()" << std::endl;
    if (key_on_device()) throw std::runtime_error("command not supported by HW wallet");
  ...
  std::vector<std::shared_ptr<monero_tx_wallet>> monero_wallet_light::create_txs(const monero_tx_config& config) {
    ...
    std::cout << "monero_wallet_light::create_txs(): spendable outs: " << unspent_outs.size() << ", total: " << unspent_outs_res.m_amount.get() << ", requested: " << amount << std::endl;
    ...
    monero_tx_wallet_light constructed_tx = create_transaction(subaddr_account_idx, dests, config.m_payment_id, sending_amounts, random_outs_params.m_change_amount, random_outs_params.m_using_fee, random_outs_params.m_using_outs, tied_outs.m_mix_outs, 0);
    
    if (constructed_tx.m_tx == boost::none) throw std::runtime_error("constructed cn tx is none");
    ptx.tx = constructed_tx.m_tx.get();
    ptx.construction_data = constructed_tx.m_construction_data.get();
    ptx.dests = ptx.construction_data.dests;
    ptx.selected_transfers = ptx.construction_data.selected_transfers;
    ptx.dust = 0;
    ptx.dust_added_to_fee = false;
    ptx.fee = constructed_tx.m_fee.get();
    ...
    if (relay) {
      try {
        std::cout << "monero_wallet_light::create_txs(): relaying tx..." << std::endl;
        ...
        if (submit_res.m_status != boost::none && submit_res.m_status == std::string("OK")) {
          std::cout << "monero_wallet_light::create_txs(): relayed tx" << std::endl;
          relayed = true;
        }
        else std::cout << "monero_wallet_light::create_txs(): tx not relayed" << std::endl;
      }
      catch(...) { }
    }
    ...
    tx->m_payment_id = config.m_payment_id; // TODO remove this
    ...
    std::cout << "monero_wallet_light::create_txs(): created unconfirmed tx" << ", spent key images: " << constructed_tx.m_spent_key_images->size() << std::endl;
    ...
    if (is_view_only()) {
      ...
      throw std::runtime_error("Multisig wallet not supported");
    }
    ...
    std::cout << "monero_wallet_light::create_txs(): created unconfirmed tx with " << tx->m_outputs.size() << " outputs and " << tx->m_inputs.size() << " inputs" << std::endl;
    ...
    if (!is_view_only() && relayed) m_tx_container.set_unconfirmed(tx);

    std::cout << "monero_wallet_light::create_txs(): created tx set" << std::endl;

    calculate_balance();

    std::cout << "monero_wallet_light::create_txs(): calculated balance" << std::endl;

    if (relayed) {
      m_wallet_listener->on_spend_txs(result);
    }

    std::cout << "monero_wallet_light::create_txs(): end" << std::endl;
  ...
  std::shared_ptr<monero_tx_wallet> get_output_transaction(const monero_light_tx_container& tx_container, monero_light_output &out, uint64_t current_height, std::vector<std::shared_ptr<monero_block>> &blocks) {
    auto transaction = tx_container.get(out);
  ...
  std::vector<std::shared_ptr<monero_output_wallet>> monero_wallet_light::get_outputs_aux(const monero_output_query& query) const {
    ...
    if (query.m_account_index != boost::none) {
      if (query.m_subaddress_index == boost::none) {
        outs = m_output_container.get(query.m_account_index.get());
      }
      else {
        outs = m_output_container.get(query.m_account_index.get(), query.m_subaddress_index.get());
      }
    }
    else {
      const auto res = get_unspent_outs(false);
      outs = res.m_outputs.get();
    }

    std::vector<std::shared_ptr<monero_output_wallet>> outputs;
    const auto current_height = m_address_txs.m_blockchain_height.get();
    ...
    for (auto &out : outs) {
      ...
      if (found_tx == txs.end()) {
        auto tx = get_output_transaction(m_tx_container, out, current_height + 1, blocks);
        output->m_tx = tx;
        txs.push_back(tx);
      }
      else {
      ...
      if (out.key_image_is_known()) {
        ...
        output->m_is_frozen = m_output_container.is_frozen(out);
      }
  ...
  std::string monero_wallet_light::export_outputs(bool all) const {
    ...
    auto outputs = m_output_container.export_outputs(m_tx_container, m_generated_key_images, all, start, count);
  ...
  std::shared_ptr<monero_key_image_import_result> monero_wallet_light::import_key_images(const std::vector<std::shared_ptr<monero_key_image>>& key_images) {
    ...
    for (size_t i = 0; i < key_images_size; i++) {
      ...
      m_output_container.set_key_image(key_images[i]->m_hex.get(), i);

      m_generated_key_images.set(key_images[i], tx_public_key, out_index, account_idx, subaddress_idx);
      
      if (check_spent) {
        if (m_tx_container.is_key_image_spent(key_images[i])) {
    ...
    if (spent_amount > 0 || unspent_amount > 0) {
      m_output_container.set(m_tx_container, get_unspent_outs(false));
      calculate_balance();
    }
    return result;
  }
  ...
  monero_light_get_random_outs_params monero_wallet_light::prepare_get_random_outs_params(const boost::optional<std::string>& payment_id_string, const std::vector<uint64_t>& sending_amounts, bool is_sweeping, uint32_t simple_priority, const std::vector<monero_light_output> &unspent_outs, uint64_t fee_per_b, uint64_t fee_quantization_mask, boost::optional<uint64_t> prior_attempt_size_calcd_fee, boost::optional<monero_light_spendable_random_outputs> prior_attempt_unspent_outs_to_mix_outs) {
    ...
    if (prior_attempt_unspent_outs_to_mix_outs != boost::none) {
      for (size_t i = 0; i < remaining_unusedOuts.size(); ++i) {
        ...
        if (prior_attempt_unspent_outs_to_mix_outs->find(*out.m_public_key) != prior_attempt_unspent_outs_to_mix_outs->end()) {
          ...
          params.m_using_outs.push_back(std::move(monero_utils::pop_index(remaining_unusedOuts, i)));
        }
      }
    }
    ...
    while (using_outs_amount < potential_total && remaining_unusedOuts.size() > 0) {
      auto out = monero_utils::pop_random_value(remaining_unusedOuts);
  ...
  tied_spendable_to_random_outs monero_wallet_light::tie_unspent_to_mix_outs(const std::vector<monero_light_output> &using_outs, std::vector<monero_light_random_outputs> mix_outs_from_server, const boost::optional<monero_light_spendable_random_outputs> &prior_attempt_unspent_outs_to_mix_outs) {
  ...
  void monero_wallet_light::init_common() {
    ...
    m_address_txs.m_transactions = std::vector<monero_light_tx>();
  ...
  void monero_wallet_light::set_unconfirmed_tx_inputs(const monero_tx_wallet_light &constructed_tx, const std::shared_ptr<monero_tx_wallet>& tx) const {
    // init inputs with key images
    const std::vector<std::string> &input_key_images = constructed_tx.m_spent_key_images.get();
    const auto &data = constructed_tx.m_construction_data.get();
    const auto &sources = data.sources;

    bool size_equal = sources.size() == input_key_images.size();
    int i = 0;
    tx->m_inputs.clear();

    for (const auto &src : sources) {
      std::shared_ptr<monero_output_wallet> input = std::make_shared<monero_output_wallet>();
      uint32_t account_idx = data.subaddr_account;
      uint32_t subaddress_idx = *next(data.subaddr_indices.begin(), i);
      input->m_tx = tx;
      input->m_account_index = account_idx;
      input->m_subaddress_index = subaddress_idx;
      input->m_amount = src.amount;
      if (size_equal)
      {
        const std::string& input_key_image = input_key_images[i];
  ...
  void monero_wallet_light::set_unconfirmed_tx_outputs(const monero_tx_wallet_light &constructed_tx, const std::shared_ptr<monero_tx_wallet>& tx_wallet) const {
    if (constructed_tx.m_tx == boost::none) throw std::runtime_error("cryptonote tx is null");
    if (constructed_tx.m_additional_tx_keys == boost::none) throw std::runtime_error("additional tx keys is null");
    if (constructed_tx.m_tx_key == boost::none) throw std::runtime_error("tx key is null");
    if (constructed_tx.m_construction_data == boost::none) throw std::runtime_error("tx construction data is null");

    uint64_t index = 0;
    const auto &data = constructed_tx.m_construction_data.get();
    const auto &destinations = data.dests;
    const auto &tx = constructed_tx.m_tx.get();
    ...
    crypto::secret_key_to_public_key(constructed_tx.m_tx_key.get(), tx_pub_key);
    std::vector<crypto::public_key> additional_tx_pub_keys;
    for (const crypto::secret_key &skey: constructed_tx.m_additional_tx_keys.get())
    {
    ...
    for (size_t i = 0; i < tx.vout.size(); ++i)
    {
      ...
      auto receiver = is_out_to_acc_precomp(subaddresses, output_public_key, derivation, additional_derivations, i, hwdev, get_output_view_tag(tx.vout[i]));
      if (!receiver)
      {
        auto output = std::make_shared<monero_output>();
        output->m_stealth_public_key = epee::string_tools::pod_to_hex(output_public_key);
        output->m_tx = tx_wallet;
        output->m_amount = destinations[i].amount;
      ...
      output->m_tx = tx_wallet;
      output->m_amount = destinations[i].amount;
      if (output->m_amount.get() == 0) std::cout << "set_unconfirmed_txs(): setting 0 amount to output!" << std::endl;
      output->m_stealth_public_key = epee::string_tools::pod_to_hex(output_public_key);
      output->m_is_spent = false;
      output->m_is_frozen = false;
      output->m_account_index = receiver->index.major;
      output->m_subaddress_index = receiver->index.minor;
      // TODO no way to get global index?
      output->m_index = i;
      crypto::key_image ki;
  ...
  monero_tx_wallet_light monero_wallet_light::create_partial_transaction(const uint32_t subaddr_account_idx, const std::unordered_map<crypto::public_key, cryptonote::subaddress_index> &subaddresses, const std::vector<cryptonote::address_parse_info> &to_addrs, const std::vector<uint64_t>& sending_amounts, uint64_t change_amount, uint64_t fee_amount, const std::vector<monero_light_output> &outputs, std::vector<monero_light_random_outputs> &mix_outs, const std::vector<uint8_t> &extra, uint64_t unlock_time, bool rct) {
    ...
    */
    ...
    for (size_t out_index = 0; out_index < outputs.size(); out_index++) {
      ...
      if (mix_outs.size() != 0) {
        ...
        for (
          ...
          if (mix_out__output.m_rct != boost::none && (*(mix_out__output.m_rct)).empty() == false) {
            rct::key commit;
            monero_utils::rct_hex_to_rct_commit(*mix_out__output.m_rct, commit);
      ...
      if (outputs[out_index].m_rct != boost::none
        ...
        rct::key commit;
        monero_utils::rct_hex_to_rct_commit(*(outputs[out_index].m_rct), commit);
      ...
      if (src.rct) {
        ...
        monero_utils::rct_hex_to_rct_commit(*(outputs[out_index].rct), parsed_commit);
    ...
    std::vector<size_t> selected_transfers =  m_output_container.get_indexes(outputs);
  ...
  monero_tx_wallet_light monero_wallet_light::create_transaction(const uint32_t subaddr_account_idx, const std::vector<std::string> &to_address_strings, const boost::optional<std::string>& payment_id_string, const std::vector<uint64_t>& sending_amounts, uint64_t change_amount, uint64_t fee_amount, const std::vector<monero_light_output> &outputs, std::vector<monero_light_random_outputs> &mix_outs, uint64_t unlock_time) {
    ...
    auto result = create_partial_transaction(
    ...
    auto txBlob = t_serializable_object_to_blob(*result.m_tx);
    ...
    // tx hash
    //monero_tx_wallet_light result;
    result.m_tx_hash_string = epee::string_tools::pod_to_hex(cryptonote::get_transaction_hash(*result.m_tx));
    // signed serialized tx
    result.m_signed_serialized_tx_string = epee::string_tools::buff_to_hex_nodelimer(cryptonote::tx_to_blob(*result.m_tx));
    // (concatenated) tx key
    // TODO is throwing Type must be trivially copiable
    crypto::secret_key _tx_key = *result.m_tx_key;
    epee::wipeable_string oss = epee::to_hex::wipeable_string(_tx_key);
    
    for (size_t i = 0; i < (*result.m_additional_tx_keys).size(); ++i) {
      oss += epee::to_hex::wipeable_string((*result.m_additional_tx_keys)[i]);
    }
    result.m_tx_key_string = std::string(oss.data(), oss.size());
    
    std::ostringstream oss2;
    oss2 << epee::string_tools::pod_to_hex(cryptonote::get_tx_pub_key_from_extra(*result.m_tx));
  ...
  std::vector<std::shared_ptr<monero_transfer>> monero_wallet_light::get_transfers_aux(const monero_transfer_query& query) const {
    ...
    for (const auto &tx : txs) {
      ...
      const bool has_payment_id = tx.m_payment_id != boost::none && !tx.m_payment_id.get().empty() && tx.m_payment_id.get() != monero_tx::DEFAULT_PAYMENT_ID;
      ...
      tx_wallet->m_hash = tx.m_hash.get();
      ...
      if (is_incoming) {
        for (auto &out : m_output_container.get_by_tx_hash(tx_hash)) {
      ...
      if (is_outgoing && !view_only) {
        ...
        outgoing_amount = total_sent - fee;
        outgoing_transfer->m_tx = tx_wallet;
        outgoing_transfer->m_amount = outgoing_amount;
        ...
        /* TODO there's some bug on lws server and payment id is not the correct one sometimes
        if (is_change && has_payment_id) {
          bool change_output_found = false;
          uint32_t recipient_account_idx = 0;
          uint32_t recipient_subaddress_idx = 0;

          if (tx.m_recipient != boost::none) {
            recipient_account_idx = *tx.m_recipient->m_maj_i;
            recipient_subaddress_idx = *tx.m_recipient->m_min_i;
          }
          ...
          for (auto &out : m_output_container.get_by_tx_hash(tx_hash)) {
            ...
            //if (!change_output_found && account_idx == recipient_account_idx && subaddress_idx == recipient_subaddress_idx) {
            //  change_output_found = true;
            //  continue;
            //}

            dest->m_address = get_address(account_idx, subaddress_idx);
            dest->m_address = get_integrated_address(dest->m_address.get(), payment_id).m_integrated_address;

            dest->m_amount = gen_utils::uint64_t_cast(*out.m_amount);
            outgoing_transfer->m_amount = outgoing_transfer->m_amount.get() + dest->m_amount.get();
            outgoing_transfer->m_destinations.push_back(dest);
          }

          if (outgoing_transfer->m_destinations.size() > 0) tx_wallet->m_payment_id = tx.m_payment_id;
        }
        */
      ...
      //if (!tx_wallet->m_is_incoming.get() && outgoing_amount > 0 && tx.m_payment_id != boost::none && tx.m_payment_id.get() != monero_tx::DEFAULT_PAYMENT_ID) tx_wallet->m_payment_id = tx.m_payment_id;
    ...
    bool added_unconfirmed = false;
    for (const auto &kv : m_tx_container.get_unconfirmed_txs()) {
      ...
      for (const std::shared_ptr<monero_transfer>& transfer : tx_wallet->filter_transfers(*_query)) {
        std::cout << "get_transfers_aux(): adding unconfirmed tx transfers" << std::endl;
        added_unconfirmed = true;
        transfers.push_back(transfer);
      }
    }

    if (!added_unconfirmed) std::cout << "get_transfers_aux(): no uncofirmed tx transfers added, with " << m_tx_container.get_unconfirmed_txs().size() << " unconfirmed txs" << std::endl; 
  ...
  uint64_t monero_wallet_light::get_subaddress_num_blocks_to_unlock(uint32_t account_idx, uint32_t subaddress_idx) const {
    auto unspent_outs = m_output_container.get(account_idx, subaddress_idx);
    return m_tx_container.calculate_num_blocks_to_unlock(unspent_outs, get_height());
  }
  ...
  bool monero_wallet_light::output_is_spent(monero_light_output &output) const {
    ...
    if (!spent && !output.key_image_is_known()) {
      try {
        ...
        spent = m_tx_container.is_key_image_spent(output.m_key_image.get());
  ...
  bool monero_wallet_light::spend_is_real(monero_light_spend &spend) const {
    ...
    if (spend.m_sender != boost::none) {
      const auto &rcpt = spend.m_sender.get();
      received_subaddr.major = rcpt.m_maj_i.get();
      received_subaddr.minor = rcpt.m_min_i.get();
    }
    else {
      received_subaddr.major = 0;
      received_subaddr.minor = 0;
    }
  ...
  void monero_wallet_light::calculate_balance() {
    m_tx_container.set(get_address_txs(), m_address_info);
    m_output_container.calculate_balance(m_tx_container, get_height());
  }
  ...
  monero_sync_result monero_wallet_light::sync_aux(boost::optional<uint64_t> start_height) {
    ...
    if (is_connected_to_daemon()) {
      ...
      try {
        ...
        auto address_txs = get_address_txs();
        // initialize optimized data structures
        m_output_container.set(m_tx_container, get_unspent_outs(false));
        m_tx_container.set(address_txs, addr_info);
        ...
        const auto txs = address_txs.m_transactions.get();
  ...
  std::string monero_wallet_light::sign_tx(tools::wallet2::unsigned_tx_set &exported_txs, std::vector<tools::wallet2::pending_tx> &txs, tools::wallet2::signed_tx_set &signed_txes) {
    ...
    for (size_t n = 0; n < exported_txs.txes.size(); ++n)
    {
      ...
      if (tx_key != crypto::null_skey)
      {
        ...
        m_tx_container.add_tx_key(tx_key, txid, additional_tx_keys);
      }
  ...
  std::string monero_wallet_light::dump_pending_tx(const monero_tx_wallet_light &tx, boost::optional<std::string> payment_id) const {
    ...
    txs.new_transfers = m_output_container.export_outputs(m_tx_container, m_generated_key_images, false, 0);
    // save as binary
    std::ostringstream oss;
    binary_archive<true> ar(oss);
    try
  ...
  bool monero_wallet_light::get_tx_key(const crypto::hash &txid, crypto::secret_key &tx_key, std::vector<crypto::secret_key> &additional_tx_keys) const {
    bool r = m_tx_container.get_tx_key(txid, tx_key, additional_tx_keys);
    if (r)
    {
  ...
  monero_light_get_address_txs_response monero_wallet_light::get_address_txs() const {
    ...
    res.m_transactions = std::vector<monero_light_tx>();

    const auto txs = *result.m_transactions;

    for(auto &_tx : txs) {
      auto tx = std::make_shared<monero_light_tx>();
      const auto __tx = std::make_shared<monero_light_tx>(_tx);
  ...
  monero_light_get_unspent_outs_response monero_wallet_light::get_spendable_outs(const uint32_t account_idx, const std::vector<uint32_t> &subaddresses_indices, uint64_t amount, uint32_t mixin, bool use_dust, uint64_t dust_threshold, bool filter_spent) const {
    ...
    for (auto &out : outputs) {
      ...
      if (m_output_container.is_frozen(out)) {
        if (_amount < out_amount) throw std::runtime_error("spendable amount is negative");
        _amount -= out_amount;
        continue;
      }

      const uint64_t unlock_time = m_tx_container.calculate_num_blocks_to_unlock(out, height);
      if (unlock_time > 0) {
        if (_amount < out_amount) throw std::runtime_error("spendable amount is negative");
        _amount -= out_amount;
        continue;
